...
from Net.layers import Conv2dWithConstraint, LazyLinearWithConstraint, PositionalEncodingFourier
"""
input dims should be :batch, 1 , num_channels, times
"""
...
class classifier(nn.Module):
    def __init__(self, num_classes,linear_input,return_fc):
        super(classifier, self).__init__()
        self.linear_input = linear_input
        self.return_fc=return_fc
        self.dense = nn.Sequential(
           # nn.Conv2d(8, num_classes, (1, 51)),
            nn.Linear(self.linear_input,num_classes),
            nn.LogSoftmax(dim=1)
        )
...
class ADFCNNNet(nn.Module):
    def __init__(self,
        ...
        super(ADFCNNNet, self).__init__()
        if sampling_rate == 1000 : # 4s data
            linear_in = 552
        elif sampling_rate == 875 : # 3.5s
            linear_in = 480
        elif sampling_rate == 1125:  # 4.5s
            linear_in = 624
        else:
            raise ValueError (" no  this inpput data!!!!")
        self.backbone = ADFCNN(num_channels=num_channels, sampling_rate=sampling_rate)

        self.classifier = classifier(num_classes,linear_in,return_fc)
...
def get_ADFCNNmodel(args):
    model = ADFCNNNet(num_classes=args.num_classes,
...
# a = torch.zeros(size=(10,1,22,1125))
# net =ADFCNNNet(4,22,1125,True)
# c,b = net(a)
# print(c.size())
# print(b.size())
# print(a)
